...
bool LocalPlayer::isWalkLocked(LocalPlayerPtr localPlayer) {
    ...
    typedef bool(gameCall* IsWalkLocked)(uintptr_t RCX);
    ...
    return g_dispatcher->scheduleEventEx([function, localPlayer]() {
        return function(reinterpret_cast<uintptr_t>(localPlayer.address));
    });
}

uint32_t LocalPlayer::getStates(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef uint32_t(gameCall* GetStates)(uintptr_t RCX);
...
double LocalPlayer::getHealth(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef double(gameCall* GetHealth)(uintptr_t RCX);
...
double LocalPlayer::getMaxHealth(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef double(gameCall* GetMaxHealth)(uintptr_t RCX);
...
double LocalPlayer::getFreeCapacity(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef double(gameCall* GetFreeCapacity)(uintptr_t RCX);
...
uint16_t LocalPlayer::getLevel(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef uint16_t(gameCall* GetLevel)(uintptr_t RCX);
...
double LocalPlayer::getMana(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef double(gameCall* GetMana)(uintptr_t RCX);
...
double LocalPlayer::getMaxMana(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef double(gameCall* GetMaxMana)(uintptr_t RCX);
...
uint32_t LocalPlayer::getManaShield(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef uint32_t(gameCall* GetManaShield)(uintptr_t RCX);
...
uint8_t LocalPlayer::getSoul(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef uint8_t(gameCall* GetSoul)(uintptr_t RCX);
...
uint16_t LocalPlayer::getStamina(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef uint16_t(gameCall* GetStamina)(uintptr_t RCX);
...
ItemPtr LocalPlayer::getInventoryItem(LocalPlayerPtr localPlayer, Otc::InventorySlot inventorySlot) {
    ...
    return g_dispatcher->scheduleEventEx([function, localPlayer, inventorySlot]() {
        ItemPtr result;
        function(reinterpret_cast<uintptr_t>(localPlayer.address), &result, inventorySlot);
        return result;
    });
}
...
bool LocalPlayer::hasEquippedItemId(LocalPlayerPtr localPlayer, uint16_t itemId, uint8_t tier) {
    if (!localPlayer) return 0;
    typedef bool(gameCall* HasEquippedItemId)(uintptr_t RCX, uint16_t itemId, uint8_t tier);
    ...
    return g_dispatcher->scheduleEventEx([function, localPlayer, itemId, tier]() {
        return function(reinterpret_cast<uintptr_t>(localPlayer.address), itemId, tier);
    });
}

uint16_t LocalPlayer::getInventoryCount(LocalPlayerPtr localPlayer, uint16_t itemId, uint8_t tier) {
    if (!localPlayer) return 0;
    typedef uint16_t(gameCall* GetInventoryCount)(uintptr_t RCX, uint16_t itemId, uint8_t tier);
...
bool LocalPlayer::hasSight(LocalPlayerPtr localPlayer, const Position &pos) {
    if (!localPlayer) return 0;
    typedef bool(gameCall* HasSight)(uintptr_t RCX, const Position *pos);
    ...
    return g_dispatcher->scheduleEventEx([function, localPlayer, pos]() {
        return function(reinterpret_cast<uintptr_t>(localPlayer.address), &pos);
    });
}

bool LocalPlayer::isAutoWalking(LocalPlayerPtr localPlayer) {
    if (!localPlayer) return 0;
    typedef bool(gameCall* IsAutoWalking)(uintptr_t RCX);
...
bool LocalPlayer::autoWalk(LocalPlayerPtr localPlayer, const Position &destination, bool retry) {
    if (!localPlayer) return 0;
    typedef bool(gameCall* AutoWalk)(uintptr_t RCX, const Position *pos, bool retry);
    ...
    return g_dispatcher->scheduleEventEx([function, localPlayer, destination, retry]() {
        return function(reinterpret_cast<uintptr_t>(localPlayer.address), &destination, retry);
    });
}
